#' makePhyloTree
#' 
#' This function is called by getLabGenealogy to turn an edge list and corresponding lengths and root node number into a tree of class \code{phylo}
#' 
...
#' @param Lengths a numeric vector of the same length as a column of \code{Edges}. Each entry corresponds to the length of that edge.
#' @param Root the number of the internal node which is to be the root (usually the number of tips + 1)
#'
#' @return An object of class \code{phylo}. When called by getLabGenealogy this represents the transmission tree from infectors to infectees.
#' 
